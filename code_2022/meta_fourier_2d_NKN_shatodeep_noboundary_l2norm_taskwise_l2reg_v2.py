...
base_dir = './l2regv2_1e-5_deep_models_trained_l2norm_taskwise_new/batch_size_%d_res_22_22_%d_%d_2D_Darcy_FNO_%d/shatodeep_samew_ntrain%d_NKN_s%d' %(batch_size,width,last_width,task_num,ntrain_pertask,s)
...
for nlayer in range(4):
        ...
        for ep in range(epochs):
            ...
            for i in range(int(ntrain_pertask/batch_size)):
                ...
                for task_idx in range(task_num):
                    ...
                    l2_norm_fc0 = sum(torch.norm(p).pow(2.0) for p in model.fc0.parameters() )
                    l2_norm_convlayer =  sum(torch.norm(p).pow(2.0) for p in model.convlayer.parameters() )
                    l2_norm_w =  sum(torch.norm(p).pow(2.0) for p in model.w.parameters() )
                    l2_norm_fc1 =  sum(torch.norm(p).pow(2.0) for p in model.fc1[task_idx].parameters() )
                    l2_norm_fc2 =  sum(torch.norm(p).pow(2.0) for p in model.fc2[task_idx].parameters() )
                    l2_norm = l2_norm_fc0+l2_norm_w+l2_norm_convlayer+l2_norm_fc1+l2_norm_fc2
                    loss = loss+l2lamda*l2_norm
                    losses.append(loss)
    ...
    optimizer_test =  Adam([{'params': model.fc2[task_num].parameters()},{'params': model.fc1[task_num].parameters()}], lr=learning_rate)
# scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=step_size, gamma=gamma)
    ...
    # meta test train last layer
        ...
        for  x,y,l2y_test_pretrain  in test_pretrain_loader:
            ...
            loss = myloss(out.view(batch_size, -1), y.view(batch_size, -1))
            l2_norm_fc1 = sum(torch.norm(p).pow(2.0) for p in model.fc1[task_num].parameters() )
            l2_norm_fc2 =sum(torch.norm(p).pow(2.0) for p in model.fc2[task_num].parameters() )
            l2_norm =l2_norm_fc1+l2_norm_fc2
            loss = loss+l2lamda*l2_norm
