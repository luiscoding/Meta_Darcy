...
base_dir = './v4_deep_models_trained_l2norm_taskwise_new/batch_size_%d_res_22_22_%d_%d_2D_Darcy_FNO_%d/shatodeep_samew_ntrain%d_NKN_s%d' %(batch_size,width,last_width,task_num,ntrain_pertask,s)
...
for nlayer in range(4):
    ...
    if cuda_flag:
        model.cuda()
    optimizer_2 =  SGD([{'params': model.fc2[:99].parameters()},{'params': model.fc1[:99].parameters()}], lr=learning_rate, weight_decay=12.5*1e-5)
    optimizer_1 = torch.optim.SGD( [{'params': model.convlayer.parameters()},{'params': model.fc0.parameters()}], lr=learning_rate, weight_decay=12.5*1e-3)
